...
M.extras = {
  ...
  base16           = { ext = "yaml", url = "https://github.com/tinted-theming/schemes", label = "Base16" },
...
function M.setup()
  ...
  -- nyx.setup({ plugins = { all = true } })
  for _, extra in ipairs(names) do
    local info = M.extras[extra]
    local plugin = require("nyx.extra." .. extra)
    for style, style_name in pairs(styles) do
      local colors, groups, opts = nyx.load({ style = style, plugins = { all = true } })
      local fname = extra
      ...
      colors["_upstream_url"] = "https://github.com/serhez/nyx.nvim/raw/main/extras/" .. fname
      colors["_style_name"] = "Nyx" .. style_name
      colors["_name"] = "nyx_" .. style
      colors["_style"] = style
      print("[write] " .. fname)
